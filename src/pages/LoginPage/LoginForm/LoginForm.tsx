...
const faGoogleIcon = faGoogle as IconProp
...
function LoginForm({ setIsLogin }: Props) {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<formValues>({ resolver: yupResolver(validateLoginSchema) });
  ...
  const handleLoginByEmail: SubmitHandler<formValues> = (data) => {
    ...
    <div className={cx('container')}>
      ...
      <div className={cx('socials-login')}>
        ...
        <ButtonBase className={cx('social-login-btn', 'google')} onClick={handleLoginGoogle}>
          <FontAwesomeIcon icon={faGoogleIcon} />
        </ButtonBase>
      </div>
    </div>
  );
